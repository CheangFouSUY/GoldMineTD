...
public class ActionBar extends Bar {
    ...
    private void initButtons() {
        ...
        // bSpeed = new MyButton("Speed", 2, 722, 100, 30);
    ...
    public void removeOneLife() { if (--lives <= 0) SetGameState(GAME_OVER); }
    ...
    public void draw(Graphics g) {
        ...
        drawDisplayedTower(g); // Display Tower
        drawWaveInfo(g); // Wave Info
        drawGoldAmount(g); // Gold Info
        if (showTowerCost) drawTowerCost(g); // Draw Tower Cost

        if (playing.isGamePaused()) { // Game Pause Text
    ...
    private boolean isTowerCostMoreThanCurrentGold() { return getTowerCostCost() > gold; }
    private String getTowerCostName() { return helpz.Constants.Towers.GetName(towerCostType); }
    private int getTowerCostCost() { return helpz.Constants.Towers.GetTowerCost(towerCostType); }
    private void drawGoldAmount(Graphics g) { g.drawString("Gold: " + gold + "g", 110, 725); }
    ...
    public void mouseClicked(int x, int y) {
        if (bMenu.getBounds().contains(x, y)) SetGameState(MENU);
        else if (bPause.getBounds().contains(x, y)) togglePause();
        // else if (bSpeed.getBounds().contains(x, y)) toggleSpeed();
        else {
            if (displayedTower != null) {
    ...
    private boolean isGoldEnoughForTower(int towerType) { return gold >= helpz.Constants.Towers.GetTowerCost(towerType); }

    public void mouseMoved(int x, int y) {
        bMenu.setMouseOver(false);
        bPause.setMouseOver(false);
        // bSpeed.setMouseOver(false);
        showTowerCost = false;
        sellTower.setMouseOver(false);
        upgradeTower.setMouseOver(false);

        for (MyButton b : towerButtons) b.setMouseOver(false);

        if (bMenu.getBounds().contains(x, y)) bMenu.setMouseOver(true);
        else if (bPause.getBounds().contains(x, y)) bPause.setMouseOver(true);
        // // else if (bSpeed.getBounds().contains(x, y)) bSpeed.setMouseOver(true);
        else {
            if (displayedTower != null) {
                if (sellTower.getBounds().contains(x, y)) {
    ...
    public void mousePressed(int x, int y) {
        ...
        else if (bPause.getBounds().contains(x, y)) bPause.setMousePressed(true);
        // // else if (bSpeed.getBounds().contains(x, y)) bSpeed.setMousePressed(true);
        else {
            if (displayedTower != null) {
                if (sellTower.getBounds().contains(x, y)) {
    ...
    public void mouseReleased(int x, int y) {
        ...
        // bSpeed.resetBooleans();
        for (MyButton b : towerButtons) b.resetBooleans();
        sellTower.resetBooleans();
        upgradeTower.resetBooleans();
    }

    public void payForTower(int towerType) { this.gold -= helpz.Constants.Towers.GetTowerCost(towerType); }
    public void addGold(int getReward) { this.gold += getReward; }
