...
public class Editing extends GameScene implements SceneMethods {
        ...
        if (selectedTile != null) {
            ...
            if (selectedTile.getId() >= 0) {
                ...
                if (game.getTileManager().getTile(id).getTileType() == ROAD_TILE) {
                    if (selectedTile.getId() == -1) start = new PathPoint(tileX, tileY);
                    else end = new PathPoint(tileX, tileY);
                }
            }
        }
    }
    ...
    public void mouseClicked(int x, int y) {
        if (y >= 640) toolbar.mouseClicked(x, y);
        else changeTile(mouseX, mouseY);
    }
    ...
    @Override
    public void mousePressed(int x, int y) { if (y >= 640) toolbar.mousePressed(x, y); }

    @Override
    public void mouseReleased(int x, int y) { toolbar.mouseReleased(x, y); }

    @Override
    public void mouseDragged(int x, int y) { if (y < 640) changeTile(x, y); }
