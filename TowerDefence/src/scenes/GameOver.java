...
public class GameOver extends GameScene implements SceneMethods {
    ...
    private void replayGame() {
        ...
        SetGameState(PLAYING); // Change State to Playing
    }

    private void resetAll() { game.getPlaying().resetEverything(); }
    ...
    public void mouseClicked(int x, int y) {
        if (bMenu.getBounds().contains(x, y)) {
            SetGameState(MENU);
            resetAll();
        } else if (bReplay.getBounds().contains(x, y)) replayGame();
    }
    ...
    public void mouseMoved(int x, int y) {
        ...
        else if (bReplay.getBounds().contains(x, y)) bReplay.setMouseOver(true);
    }
