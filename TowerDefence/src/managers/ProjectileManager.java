...
public class ProjectileManager {
    ...
    private void importImgs() {
        ...
        for (int i = 0; i < 3; i++) proj_imgs[i] = atlas.getSubimage((7 + i) * 32, 32, 32, 32);
        importExplosion(atlas);
    }

    private void importExplosion(BufferedImage atlas) {
        explo_imgs = new BufferedImage[7];
        for (int i = 0; i < 7; i++) explo_imgs[i] = atlas.getSubimage(i * 32, 32 * 2, 32, 32);
    }
    ...
    public void update() {
        for (Projectile p : projectiles)
            if (p.isActive()) {
                ...
                } else if (isProjOutsideBounds(p)) p.setActive(false);
            }
        for (Explosion e : explosions)
            if (e.getIndex() < 7) e.update();
    }
        ...
        for (Enemy e : playing.getEnemyManger().getEnemies()) {
            if (e.isAlive())
                if (e.getBounds().contains(p.getPos())) {
                    e.hurt(p.getDmg());
                    if (p.getProjectileType() == CHAINS) e.slow();
                    return true;
                }
        }
        return false;
    ...
    private boolean isProjOutsideBounds(Projectile p) {
        if (p.getPos().x >= 0)
            if (p.getPos().x <= 640)
                if (p.getPos().y >= 0)
                    if (p.getPos().y <= 800) return false;
        return true;
    }
    ...
    private int getProjType(Tower t) {
        switch (t.getTowerType()) {
            ...
            case CANNON: return BOMB;
            case WIZARD: return CHAINS;
        }
        return 0;
    }
    ...
    public class Explosion {
        ...
        public Explosion(Point2D.Float pos) { this.pos = pos; }
        public void update() {
            exploTick++;
            if (exploTick >= 6) {
                exploTick = 0;
        ...
        public int getIndex() { return exploIndex; }
