...
public class Utilz {
    ...
    private static int GetDirFromPrevToCurr(PathPoint prevTile, PathPoint currTile) {
        ...
        if (prevTile.getxCord() == currTile.getxCord()) {
            if (prevTile.getyCord() > currTile.getyCord()) return UP;
            else return DOWN;
        } else {
            // Right or left
            if (prevTile.getxCord() > currTile.getxCord()) return LEFT;
            else return RIGHT;
        }
    ...
    private static boolean IsTileRoad(PathPoint testTile, int[][] lvlTypeArr) {
        if (testTile != null && testTile.getyCord() >= 0 && testTile.getyCord() < lvlTypeArr.length && testTile.getxCord() >= 0 && testTile.getxCord() < lvlTypeArr[0].length && lvlTypeArr[testTile.getyCord()][testTile.getxCord()] == ROAD_TILE)
            return true;
        return false;
    }
    ...
    private static boolean IsCurrSameAsEnd(PathPoint currTile, PathPoint end) {
        if (currTile.getxCord() == end.getxCord())
            if (currTile.getyCord() == end.getyCord()) return true;
        return false;
    }
